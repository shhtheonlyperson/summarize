// Inlined CSS, JS, and SVG assets for the summarize docs site.
// The visual identity is summarize's: warm paper + dark backdrop, with
// lime / orange / teal accents. The brand mark stacks three bars whose
// width shrinks — a literal "summarize" gesture.
...
:root{
  ...
  --paper:#ffffff;
  --accent:#1f8a78;
  --accent-soft:rgba(31,138,120,.10);
  --accent-strong:#176658;
  --s-lime:#7fc52d;
  --s-orange:#e87a1c;
  --s-teal:#2aa192;
  --line:#e3dccb;
...
:root[data-theme="dark"]{
  ...
  --paper:#131820;
  --accent:#b8ff5a;
  --accent-soft:rgba(184,255,90,.16);
  --accent-strong:#cfff8a;
  --s-lime:#b8ff5a;
  --s-orange:#ff9b35;
  --s-teal:#36c9b9;
  --line:#23282f;
...
body{margin:0;background:var(--bg);color:var(--text);font-family:"IBM Plex Sans",ui-sans-serif,system-ui,-apple-system,Segoe UI,sans-serif;line-height:1.65;overflow-x:hidden;-webkit-font-smoothing:antialiased;transition:background-color .18s,color .18s}
::selection{background:var(--accent);color:#0b0f12}
a{color:var(--accent-strong);text-decoration:none;transition:color .12s}
...
.brand .mark{display:flex;flex-direction:column;justify-content:center;gap:4px;flex:0 0 30px;height:30px}
.brand .mark i{display:block;height:5px;border-radius:2px}
.brand .mark i:nth-child(1){width:30px;background:var(--s-lime)}
.brand .mark i:nth-child(2){width:22px;background:var(--s-orange)}
.brand .mark i:nth-child(3){width:14px;background:var(--s-teal)}
.brand strong{display:block;font-size:1.05rem;line-height:1.1;font-weight:600;letter-spacing:0;color:var(--ink);font-family:"Fraunces",serif}
...
nav h2{font-size:.66rem;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;margin:0 0 6px;font-weight:600;display:flex;align-items:center;gap:6px}
nav h2::before{content:"";display:inline-block;width:6px;height:6px;border-radius:2px;background:var(--accent)}
nav section:nth-of-type(1) h2::before{background:var(--s-lime)}
nav section:nth-of-type(2) h2::before{background:var(--s-orange)}
nav section:nth-of-type(3) h2::before{background:var(--s-teal)}
nav section:nth-of-type(4) h2::before{background:var(--s-lime)}
nav section:nth-of-type(5) h2::before{background:var(--s-orange)}
nav section:nth-of-type(6) h2::before{background:var(--s-teal)}
...
:root[data-theme="dark"] .nav-link.active{color:var(--accent)}
...
.eyebrow{margin:0 0 8px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.08em;font-size:.7rem}
.hero h1{font-size:2.25rem;line-height:1.1;letter-spacing:-.01em;margin:0;font-weight:700;color:var(--ink);font-family:"Fraunces",serif}
...
.home-hero{padding:14px 0 30px;margin-bottom:8px;border-bottom:1px solid var(--line);position:relative}
.home-hero::after{content:"";position:absolute;left:0;right:auto;bottom:-1px;height:3px;width:160px;background:linear-gradient(90deg,var(--s-lime),var(--s-orange) 50%,var(--s-teal));border-radius:3px}
.home-hero h1{font-size:3.25rem;line-height:1.04;letter-spacing:-.02em;margin:0 0 .35em;font-weight:700;color:var(--ink);font-family:"Fraunces",serif}
...
.home-cta .btn-primary:hover{background:var(--accent-strong);border-color:var(--accent-strong);color:#fff;text-decoration:none}
:root[data-theme="dark"] .home-cta .btn-primary{background:var(--accent);color:#0b0f12;border-color:var(--accent)}
:root[data-theme="dark"] .home-cta .btn-primary:hover{background:var(--accent-strong);border-color:var(--accent-strong);color:#0b0f12}
.home-cta .btn-ghost{padding:10px 16px}
.home-install{display:flex;align-items:center;gap:12px;background:var(--code-bg);color:var(--code-fg);border-radius:8px;padding:10px 10px 10px 16px;font:500 .9rem/1.2 "IBM Plex Mono","JetBrains Mono",ui-monospace,monospace;max-width:32em;border:1px solid var(--code-border)}
...
.home-install .copy{flex:0 0 auto;background:rgba(255,255,255,.08);color:var(--code-fg);border:1px solid rgba(255,255,255,.16);border-radius:6px;padding:5px 11px;font:500 .72rem/1 "IBM Plex Sans",sans-serif;cursor:pointer;transition:background .15s,border-color .15s}
.home-install .copy:hover{background:rgba(255,255,255,.16)}
.home-install .copy.copied{background:var(--accent);color:#0b0f12;border-color:var(--accent)}
.home-modes{display:flex;flex-wrap:wrap;gap:6px;margin:14px 0 8px}
.home-modes span{display:inline-flex;align-items:center;gap:6px;padding:3px 10px;border:1px solid var(--pill-border);border-radius:999px;font-size:.78rem;color:var(--text);background:var(--paper);font-weight:500}
.home-modes span::before{content:"";display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--s-lime)}
.home-modes span.m-orange::before{background:var(--s-orange)}
.home-modes span.m-teal::before{background:var(--s-teal)}
...
.doc h1{font-size:2.6rem;line-height:1.08;letter-spacing:-.01em;margin:0 0 .4em;font-weight:700;color:var(--ink);font-family:"Fraunces",serif}
body:not(.home) .doc>h1:first-child{display:none}
.doc h2{font-size:1.45rem;line-height:1.2;margin:2em 0 .5em;font-weight:600;letter-spacing:0;color:var(--ink);position:relative;font-family:"Fraunces",serif}
...
.doc code{font-family:"IBM Plex Mono","JetBrains Mono",ui-monospace,monospace;font-size:.84em;background:var(--line-soft);border:1px solid var(--line);border-radius:5px;padding:.08em .35em;color:var(--code-inline-fg)}
...
.doc pre code{display:block;background:transparent;border:0;color:inherit;padding:0;font-size:1em;white-space:pre;font-family:"IBM Plex Mono","JetBrains Mono",ui-monospace,monospace}
.doc pre .copy{position:absolute;top:8px;right:8px;background:rgba(255,255,255,.06);color:var(--code-fg);border:1px solid rgba(255,255,255,.16);border-radius:6px;padding:3px 9px;font:500 .7rem/1 "IBM Plex Sans",sans-serif;cursor:pointer;opacity:0;transition:opacity .15s,background .15s,border-color .15s}
.doc pre:hover .copy,.doc pre .copy:focus{opacity:1}
.doc pre .copy:hover{background:rgba(255,255,255,.12)}
.doc pre .copy.copied{background:var(--accent);color:#0b0f12;border-color:var(--accent);opacity:1}
...
.doc pre .hl-cmd{color:var(--hl-string);font-weight:600}
...
.toc a.active{color:var(--accent-strong);border-left-color:var(--accent);font-weight:500}
:root[data-theme="dark"] .toc a.active{color:var(--accent)}
...
.page-nav small{display:block;color:var(--muted);font-size:.7rem;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px;font-weight:600}
...
export function faviconSvg() {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" role="img" aria-label="summarize">
<rect width="64" height="64" rx="12" fill="#0b0f12"/>
<rect x="14" y="20" width="36" height="6" rx="3" fill="#b8ff5a"/>
<rect x="14" y="29" width="28" height="6" rx="3" fill="#ff9b35"/>
<rect x="14" y="38" width="20" height="6" rx="3" fill="#36c9b9"/>
</svg>`;
}
