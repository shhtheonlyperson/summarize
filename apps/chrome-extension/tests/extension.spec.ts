...
test("sidepanel shows a loading state instead of going blank while waiting for auto summarize", async ({
  ...
  try {
    ...
    await expect(page.locator("#render")).toContainText("正在準備摘要");
    await expect(page.locator(".renderEmpty__label")).toHaveText("載入中");
...
test("sidepanel switches between page, video, and slides modes", async ({
  ...
  try {
    ...
    await expect(summarizeButton).toHaveAttribute("aria-label", /頁面(?: · 120 字)?/);

    await setSummarizeMode("page", false);
    await expectSummarizeMode("page", false);
    await expect(summarizeButton).toHaveAttribute("aria-label", /頁面/);
    await expect(
    ...
    await expect(summarizeButton).toHaveAttribute("aria-label", /影片/);
    await expect(
      page.locator("img.slideStrip__thumbImage, img.slideInline__thumbImage"),
    ).toHaveCount(0);
    ...
    await expect(summarizeButton).toHaveAttribute("aria-label", /投影片/);
